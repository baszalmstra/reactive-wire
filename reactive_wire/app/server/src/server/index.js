...
let ha;
let stopSim = () => { };
function describeHaConnectError(err) {
    if (err === 1)
        return "cannot connect to Home Assistant; check HA_URL is reachable from this process";
    if (err === 2)
        return "invalid Home Assistant token";
    if (err === 4)
        return "HA_URL is required";
    if (err === 5)
        return "browser HTTPS to Home Assistant HTTP is not allowed by the HA websocket client";
...
if (deployToken)
    log("info", "server", "deploy/control WebSocket requires RW_DEPLOY_TOKEN");
if (trustedIngress && !deployToken)
    log("info", "server", "trusting upstream ingress authentication for deploy/control WebSocket");
if (staticDir)
    log("info", "server", "serving editor frontend", { path: staticDir });
