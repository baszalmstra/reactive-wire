import { evaluate, isSink, sinkCalls } from "../../shared/engine/evaluate.js";
 ...
 */
function debugCall(call) {
    const data = {};
    for (const [k, v] of Object.entries(call.data))
        data[k] = jsonSafe(v);
    return call.target
...
const noFetch = () => Promise.reject(new Error("no fetch configured"));
/**
...
export class Deployer {
    ...
    generation = 0;
    inFlight = new Map();
    lastNonReconciling = new Map();
    tick;
    poller;
    lastResults = null;
    lastRunAt = null;
    constructor(ha, tickMs = 1000, fetchFn = noFetch, durable) {
        this.ha = ha;
     ...
     */
    deploy(nodes, edges, actuate, macros = {}) {
    ...
    run() {
        if (!this.graph)
            return;
        const entities = this.ha.entitiesSnapshot();
        ...
        for (const { nodeId, call } of sinkCalls(this.graph.nodes, results)) {
            ...
            if (this.inFlight.get(nodeId) === key)
                continue;
            if (rememberUntilChange && this.lastNonReconciling.get(nodeId) === key)
                continue;
    ...
    async executeCall(generation, nodeId, key, call, rememberUntilChange) {
        const targetId = call.target?.entity_id ?? "";
        try {
