...
function asRecord(v) {
    const plain = toPlain(v);
    return typeof plain === "object" && plain !== null && !Array.isArray(plain) ? plain : null;
}
function safeString(v, fallback = "", max = 240) {
...
function safeJson(value, depth = 0) {
    ...
    if (Array.isArray(plain))
        return plain.slice(0, MAX_ARRAY_ITEMS).map((x) => safeJson(x, depth + 1));
...
function sanitizeNode(raw) {
    ...
    if (!id)
        return null;
    const node = safeJson({ ...record, id, selected: undefined });
    node.id = id;
...
function sanitizeEdge(raw) {
    ...
    if (!id || !source || !target)
        return null;
    const edge = safeJson({ ...record, id, source, target });
    edge.id = id;
    edge.source = source;
    edge.target = target;
    return edge;
}
function sanitizeMacroMap(raw) {
    const record = asRecord(raw);
    if (!record)
        return {};
    const out = {};
    ...
    for (const [key, value] of Object.entries(record)) {
        if (count++ >= MAX_MACROS)
            break;
        if (!SAFE_KEY.test(key) || !isRecord(value))
            continue;
        const macro = safeJson(value);
        const id = safeString(macro.id, key).trim();
        if (!id)
            continue;
        out[id] = { ...macro, id };
    }
...
export function emptyEditorDocumentSnapshot(flowId = DEFAULT_FLOW_ID) {
    return {
        version: VERSION,
        activeFlowId: flowId,
...
export function sanitizeEditorDocumentSnapshot(raw) {
    const record = asRecord(raw);
    if (!record)
        return emptyEditorDocumentSnapshot();
    ...
    for (const f of flowsRaw) {
        ...
        if (!id || flows.some((x) => x.id === id))
            continue;
        const nodes = (Array.isArray(flowRecord.nodes) ? flowRecord.nodes : []).slice(0, MAX_NODES_PER_FLOW).map(sanitizeNode).filter((n) => !!n);
    ...
    if (flows.length === 0)
        flows.push(emptyEditorDocumentSnapshot().flows[0]);
...
function flowMaps(flow) {
    ...
    let edgeOrder = flow.get("edgeOrder");
    if (!(edgeOrder instanceof Y.Array)) {
        edgeOrder = new Y.Array();
        flow.set("edgeOrder", edgeOrder);
    }
    return { nodes, nodeOrder, edges, edgeOrder };
}
function replaceArray(arr, values) {
    const current = arr.toArray();
    if (current.length === values.length && current.every((value, i) => Object.is(value, values[i])))
        return;
    if (arr.length)
...
function setMapValueIfChanged(target, key, value) {
    if (JSON.stringify(safeJson(target.get(key)) ?? null) !== JSON.stringify(safeJson(value) ?? null))
        target.set(key, value);
}
function syncMapValue(target, key, previous, next) {
    ...
    if (JSON.stringify(safeJson(previous) ?? null) !== JSON.stringify(value))
        setMapValueIfChanged(target, key, value);
}
function syncMapObject(target, previous, next) {
...
function setObjectEntry(map, id, previous, next) {
...
function syncOrder(order, previousIds, nextIds) {
...
function updateMapFromDiff(map, order, previous, next) {
    ...
    for (const item of next) {
        ...
        if (JSON.stringify(old ?? null) !== JSON.stringify(item))
            setObjectEntry(map, item.id, old ?? {}, item);
    }
...
export function ensureEditorDocInitialized(doc) {
    ...
    if (version !== VERSION)
        throw new UnsupportedEditorDocumentVersionError(version);
}
export function applyEditorSnapshot(doc, rawSnapshot, origin) {
    ...
    doc.transact(() => {
        ...
        for (const id of Array.from(flows.keys()))
            flows.delete(id);
        ...
        for (const flow of snapshot.flows) {
            ...
            for (const node of flow.nodes)
                setObjectEntry(nodes, node.id, {}, node);
            replaceArray(nodeOrder, flow.nodes.map((n) => n.id));
            for (const edge of flow.edges)
                setObjectEntry(edges, edge.id, {}, edge);
            replaceArray(edgeOrder, flow.edges.map((e) => e.id));
        }
        for (const id of Array.from(macros.keys()))
            macros.delete(id);
        for (const [id, macro] of Object.entries(snapshot.macros))
            setObjectEntry(macros, id, {}, macro);
    }, origin);
    return snapshot;
}
export function applyEditorSnapshotDiff(doc, previousRaw, nextRaw, origin) {
    ...
    doc.transact(() => {
        ...
        for (const id of prevFlows.keys())
            if (!nextFlows.has(id))
                flows.delete(id);
        for (const flow of next.flows) {
            let yFlow = flows.get(flow.id);
        ...
        for (const id of Object.keys(prevMacros))
            if (!(id in next.macros))
                macros.delete(id);
        for (const [id, macro] of Object.entries(next.macros)) {
            if (JSON.stringify(prevMacros[id] ?? null) !== JSON.stringify(macro))
                setObjectEntry(macros, id, prevMacros[id] ?? {}, macro);
        }
    }, origin);
    return next;
}
function orderedValues(map, order, sanitize) {
    const seen = new Set();
    const out = [];
...
export function readEditorDocumentVersion(doc) {
 ...
 */
export function readEditorDocumentSnapshotUnchecked(doc) {
    return readEditorDocSnapshot(doc);
}
export function snapshotFromEditorDoc(doc) {
    ensureEditorDocInitialized(doc);
    return readEditorDocSnapshot(doc);
}
function readEditorDocSnapshot(doc) {
    const { meta, flows, flowOrder, macros } = rootMaps(doc);
    const seen = new Set();
    const outFlows = [];
    const readFlow = (id) => {
        const yFlow = flows.get(id);
        if (!(yFlow instanceof Y.Map) || seen.has(id))
            return;
        seen.add(id);
        const { nodes, nodeOrder, edges, edgeOrder } = flowMaps(yFlow);
    ...
    for (const id of flowOrder.toArray())
        readFlow(id);
    for (const id of Array.from(flows.keys()).sort())
        readFlow(id);
    const macroObj = {};
...
export function encodeUpdateBase64(update) {
    ...
    if (maybeBuffer)
        return maybeBuffer.from(update).toString("base64");
    let binary = "";
    for (const byte of update)
        binary += String.fromCharCode(byte);
    return btoa(binary);
}
export function decodeUpdateBase64(update, maxBytes = DEFAULT_MAX_DOC_UPDATE_BYTES) {
