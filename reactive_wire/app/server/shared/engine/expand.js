import { MACRO_IN, MACRO_OUT, isMacroInstance } from "../macros.js";
/**
...
export function joinPath(prefix, innerId) {
 ...
 */
export function expandMacros(nodes, edges, macros) {
    const outNodes = [];
    const outEdges = [];
    const instances = {};
...
function expandInto(nodes, edges, macros, prefix, active, outNodes, outEdges, instances, boundary) {
    ...
    const local = (id) => joinPath(prefix, id);
 ...
 */
function resolveSource(from, macroIds, bindings, boundary, local) {
 ...
 */
function resolveTarget(to, macroIds, bindings, local) {
 ...
 */
function expandInstance(def, inst, instPath, macros, active, outNodes, outEdges, instances) {
    ...
    const binding = { prefix: instPath, outputs: {}, inputs: {} };
    ...
    for (const b of boundaryOut) {
        for (const pin of b.inputs) {
            if (!(pin.id in binding.outputs))
                binding.outputs[pin.id] = null;
        }
    }
    return binding;
}
/** A two-pin identity node: its output equals its input, used to fan a macro input inward. */
function passthroughNode(id, pin, type, literal) {
    return {
        id,
        type: "passthrough",
        title: "",
