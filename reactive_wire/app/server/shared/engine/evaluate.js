...
import { isMacroInstance } from "../macros.js";
import { REGISTRY } from "./nodes/index.js";
import { inputHelperType, statePolicy, } from "./engine-support.js";
/** Whether a node type names a sink that actuates Home Assistant. */
export function isSink(type) {
 ...
 */
export function isTransientSink(type) {
 ...
 */
function describeSink(n, results) {
    const blocked = sinkCommandStatus(n, results);
    if (blocked)
        return blocked;
    const call = results.sinks[n.id];
    if (!call)
        return { call: null, note: "holds — no change", status: "ok" };
 ...
 */
function sinkCommandStatus(n, results) {
    ...
    if (!cmd || cmd.status === "unavailable")
        return { call: null, note: `${cmdPin} = unavailable — no call`, status: "unavailable" };
    if (cmd.status === "error")
        return { call: null, note: `${cmdPin} = error — no call`, status: "error" };
    return null;
}
/** An input pin's ok value, or null if it is unset / non-ok. Used to read a desired dimension. */
function okInput(results, nodeId, pinId) {
 ...
 */
function buildSinkCall(n, def, results, entities, memory) {
 ...
 */
export function evaluate(nodes, edges, entities, memory, now = Date.now(), sources = {}, macros = {}) {
    ...
    function resolveType(n, declared, fallbackPins) {
        if (declared !== "any")
            return declared;
    ...
    function resolveGroupType(n, fallback) {
    ...
    nodes.forEach((n) => {
        ...
        n.outputs.forEach((p) => {
            ...
            if (p.ghost || (v && v.status === "error"))
                h = "error";
            else if (h !== "error" && v && (v.status === "unavailable" || v.status === "stale"))
                h = "warn";
        });
        n.inputs.forEach((p) => {
            ...
            if (v && v.status === "error")
                h = "error";
            else if (h === "ok" && v && (v.status === "unavailable" || v.status === "stale"))
                h = "warn";
        });
        health[n.id] = h;
    });
    const results = { outputs, inputs, health, actions: {}, connected, sinks: {} };
...
function evaluateWithMacros(nodes, edges, entities, memory, now, sources, macros) {
    ...
    for (const n of nodes) {
        ...
        for (const id in inner.health) {
            if (id !== n.id && !id.startsWith(prefix))
                continue;
            const ih = inner.health[id];
            if (ih === "error")
                h = "error";
            else if (ih === "warn" && h === "ok")
                h = "warn";
        }
        health[n.id] = h;
    }
 ...
 */
export function sinkCalls(nodes, results) {
    const out = [];
    for (const n of nodes) {
        if (!isSink(n.type))
            continue;
