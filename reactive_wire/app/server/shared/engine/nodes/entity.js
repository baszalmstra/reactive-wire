...
function durationUnitSeconds(count, unit) {
    switch (String(unit)) {
 ...
 */
function stateValue(entityId, e) {
    const type = entityStateType(entityId, e.state, e.attributes);
    if (type === "duration") {
        const parsed = parseEntityValue(e.state, "num");
        if (parsed.status !== "ok")
            return UN("duration");
        return V("duration", durationUnitSeconds(parsed.v, e.attributes.unit_of_measurement));
    }
...
export const entity = {
    ...
    eval: ({ n, pinId, cfg, entities }) => {
        ...
        if (pin?.ghost)
            return ER(pin.type, `attribute '${pin.missing}' no longer exposed`);
        ...
        if (pinId === "state") {
            if (!e)
                return UN(entityStateType(entityId, "", {}));
        ...
        if (pinId === "last_changed")
            return e.last_changed == null ? UN("datetime") : V("datetime", e.last_changed);
        if (pinId === "last_updated")
            return e.last_updated == null ? UN("datetime") : V("datetime", e.last_updated);
