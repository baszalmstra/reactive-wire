 ...
 */
function buildTransientCall({ n, cfg, okInput, mem }) {
    const msg = okInput("message");
    const m = mem();
    if (m.prevVal === undefined)
        m.prevVal = null;
    if (m.seeded === undefined)
        m.seeded = false;
    ...
    if (n.type === "sink-tts") {
        ...
        const data = { message: msg.v };
        const entityId = String(cfg.entity_id ?? "");
        if (entityId)
            data.media_player_entity_id = entityId;
