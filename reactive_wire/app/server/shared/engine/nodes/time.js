...
export const datetimeSubtract = {
    ...
    eval: ({ inEff }) => {
        ...
        if (!a || !b)
            return UN("duration");
        if (a.status === "error")
            return ER("duration", a.msg);
        if (b.status === "error")
            return ER("duration", b.msg);
        if (a.status !== "ok" || b.status !== "ok")
            return UN("duration");
        return V("duration", instantDiffSeconds(toNumber(a.v, 0), toNumber(b.v, 0)));
    },
};
export const datetimeShift = {
    type: "dt-shift",
    ...
    eval: ({ cfg, inEff }) => {
        ...
        if (!t || !by)
            return UN("datetime");
        if (t.status === "error")
            return ER("datetime", t.msg);
        if (by.status === "error")
            return ER("datetime", by.msg);
        if (t.status !== "ok" || by.status !== "ok")
            return UN("datetime");
