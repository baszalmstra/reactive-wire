 ...
 */
function evalKleene(isAnd, { conn, inVal }) {
    if (conn.length === 0)
        return V("bool", isAnd);
    const vals = conn.map((p) => inVal(p.id));
    const decisive = isAnd ? false : true;
    if (vals.some((v) => v?.status === "ok" && v.v === decisive))
        return V("bool", decisive);
    const notOk = vals.filter((v) => !v || v.status !== "ok");
    if (notOk.some((v) => v?.status === "error"))
        return ER("bool", "input error");
    if (notOk.length > 0)
        return UN("bool");
    return V("bool", !decisive);
}
