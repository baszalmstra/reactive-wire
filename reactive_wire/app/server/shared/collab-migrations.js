...
import { EDITOR_DOCUMENT_VERSION, readEditorDocumentSnapshotUnchecked, readEditorDocumentVersion, } from "./collab.js";
/**
 ...
 */
export function migrateSnapshot(snapshot, fromVersion, target = EDITOR_DOCUMENT_VERSION, registry = SNAPSHOT_MIGRATIONS) {
    let current = snapshot;
    for (let v = fromVersion; v < target; v += 1) {
        const step = registry[v];
        if (!step)
            throw new Error(`No editor document migration registered from version ${v}`);
 ...
 */
export function readMigratableSnapshot(doc) {
