export const V = (type, v) => ({ type, v, status: "ok" });
export const UN = (type) => ({ type, v: null, status: "unavailable" });
export const ER = (type, msg) => ({ type, v: null, status: "error", msg });
export const ST = (type, v) => ({ type, v, status: "stale" });
...
export function formatValue(value) {
    ...
    if (value.status === "unavailable")
        return { text: "unavailable", kind: "unavail" };
...
export function formatDuration(secondsValue) {
    ...
    if (!Number.isFinite(s))
        return String(secondsValue);
    const abs = Math.abs(s);
    const round = (x) => (Math.round(x * 10) / 10).toString();
    if (abs < 1)
        return `${round(s * 1000)} ms`;
    if (abs < 60)
        return `${round(s)} s`;
    if (abs < 3600)
...
function sniffsNumeric(raw) {
    if (raw == null)
        return false;
    if (typeof raw === "string" && ABSENT.has(raw.toLowerCase()))
        return false;
    return Number.isFinite(Number(raw));
}
/** Whether a raw value reads as one of the known on/off-style boolean words. */
function sniffsBool(raw) {
 ...
 */
export function entityStateType(entityId, state, attributes) {
    ...
    if (domain === "binary_sensor")
        return "bool";
    if (deviceClass) {
        if (TIMESTAMP_CLASSES.has(deviceClass))
            return "datetime";
        if (DURATION_CLASSES.has(deviceClass))
            return "duration";
        if (ENUM_CLASSES.has(deviceClass))
            return "str";
    ...
    if (sniffsColor(state))
        return "color";
    if (sniffsBool(state))
        return "bool";
    if (sniffsNumeric(state))
        return "num";
    return "str";
}
/** Interpret a raw Home Assistant state/attribute value as the pin's declared type. */
export function parseEntityValue(raw, type) {
    if (raw == null)
        return UN(type);
    if (typeof raw === "string" && ABSENT.has(raw.toLowerCase()))
        return UN(type);
    switch (type) {
        ...
        case "datetime": {
            ...
            if (typeof raw === "number")
                return Number.isFinite(raw) ? V("datetime", raw) : UN("datetime");
            const ms = Date.parse(String(raw));
            return Number.isFinite(ms) ? V("datetime", ms) : UN("datetime");
        }
        case "color": {
            if (Array.isArray(raw) && raw.length >= 3)
                return V("color", rgbToHex(Number(raw[0]), Number(raw[1]), Number(raw[2])));
