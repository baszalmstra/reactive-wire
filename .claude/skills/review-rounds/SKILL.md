...
For a large change, one reviewer either misses cross-cutting issues or produces an unranked pile.
Split the work, then converge.

## Round 1 — parallel domain-scoped reviewers

Launch several reviewers at once, each scoped to one domain so coverage is broad and non-
overlapping. Scope to the parts of the diff that exist; typical domains for this repo:
...
- **tests + docs** — `test/`, README/DESIGN/CLAUDE: coverage of new behavior, doc accuracy.
