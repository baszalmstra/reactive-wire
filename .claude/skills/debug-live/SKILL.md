...
```sh
node .claude/skills/debug-live/scripts/query-state.mjs    # honors RW_HOST/RW_PORT/RW_DEPLOY_TOKEN
```
...
```sh
# git-bash / Linux
RW_DATA_DIR=.rw-data npx tsx .claude/skills/debug-live/scripts/decode-doc.mjs
```
```powershell
# PowerShell (Windows)
$env:RW_DATA_DIR = ".rw-data"; npx tsx .claude/skills/debug-live/scripts/decode-doc.mjs
```
