...
Climb only as far as the change reaches. Do not run e2e for a change unit tests already cover.
...
- When a change has a runtime surface, also confirm the actual behavior (see the `debug-live`
  skill to query a running server) — passing tests plus a real observation, not tests alone.
