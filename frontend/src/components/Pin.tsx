...
import { portTone } from "./port-style.js";
...
export function Pin({ side, pin, nodeId, value, anatomy, pinShape = "round", hot, onPinDown, onPinUp }: PinProps) {
  ...
  const tone = portTone(pin, value);
  const isAny = pin.type === "any" && !ghost;
  const style: CSSProperties = { ["--tc" as string]: tone.color };
  ...
  const knob = (
    <span
      ...
      className={cn(
        "rw-pin-knob absolute w-[11px] h-[11px] rounded-full z-[3] cursor-crosshair border-[2.5px] border-rw-node bg-[var(--tc)]",
        tone.className,
        !tone.className && "shadow-[0_0_0_1px_var(--tc)]",
        ...
        !tone.className && "hover:shadow-[0_0_0_1px_var(--tc),0_0_0_5px_color-mix(in_oklab,var(--tc)_28%,transparent)]",
      )}
    />
  );
