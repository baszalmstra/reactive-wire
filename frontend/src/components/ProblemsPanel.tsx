...
function ProblemRow({ p, onFocus }: { p: Problem; onFocus: (id: string) => void }) {
  return (
    <button
      ...
      className={cn(
        "rw-problem-row group w-full min-h-[44px] flex items-center gap-2 px-2.5 py-2 rounded-md text-left text-[11.5px] border transition-colors cursor-pointer",
    ...
    >
      ...
      <span className="rw-problem-copy min-w-0 flex-1 flex items-baseline gap-2">
        <span className="rw-problem-title font-mono text-rw-text shrink-0 max-w-[110px] truncate">{p.title}</span>
        <span className="rw-problem-message text-rw-dim flex-1 min-w-0 truncate">{p.message}</span>
      </span>
      <span className="rw-problem-focus text-rw-faint text-[10px] opacity-0 group-hover:opacity-100 shrink-0">focus →</span>
    </button>
  );
}
...
/** A responsive list of structural (edit-time) and runtime (live) problems; each row focuses its node. */
export function ProblemsPanel({
  ...
  return (
    <div className="rw-problems-panel absolute left-3 right-3 bottom-3 z-20 max-h-[42%] flex flex-col rounded-xl border border-rw-line bg-rw-panel shadow-rw overflow-hidden">
      <div className="flex items-center gap-2 px-3 min-h-12 border-b border-rw-line-soft shrink-0">
        ...
        <button
          onClick={onClose}
          className="w-11 h-11 inline-flex items-center justify-center rounded-md text-rw-dim hover:bg-rw-panel2 hover:text-rw-text cursor-pointer"
      ...
      <div className="rw-problems-content flex gap-3 p-3 overflow-auto">
        <Column title="Structural · edit-time" problems={structural} onFocus={onFocus} />
        <div className="rw-problems-divider w-px self-stretch bg-rw-line-soft" />
